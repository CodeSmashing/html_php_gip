        ...
        <div class="process">
            ...
            ?>
                ...
                if ($stmt->execute([$logged_user_name])) {
                    ...
                    // We gaan proberen all de huidige informatie van de gebruiker te selecteren uit de database op basis van ID's
                    $infoUser = null;
                    $testResult = 0;
                    for ($i = 1; $i <= 4; $i++) {
                        switch ($i) {
                    ...
                    // Er zijn vijf mogelijke resultaten en we baseren wat we doen op dit resultaat.
                    // Als er een gebruiker, klant, adres en bestelling is:
                    if ($testResult = 1) {
                ?>
                        <form method="post">
                            <input type="text" name="voornaam" placeholder="Voornaam*" pattern="[a-zA-Z]+(?:\s+[a-zA-Z]+)*" required></input><br>
                            <input type="text" name="achternaam" placeholder="Familienaam*" pattern="[a-zA-Z]+(?:\s+[a-zA-Z]+)*" required></input><br><br>

                            <input type="email" name="email" placeholder="Email" pattern="^[^\s@]+@[^\s@]+\.[^\s@]+$"></input><br>
                            <input type="text" name="straat" placeholder="Straatnaam*" pattern="[a-zA-Z0-9\s]+" required></input><br>
                            <input type="number" name="nummer" placeholder="Huisnummer*" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" required></input><br>
                            <input type="number" name="postcode" placeholder="Postcode*" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" required></input><br><br>

                            <button type="submit" name="customerInfo" class="send_btn" formtarget="_self">Bestelling
                                plaatsen</button>
                        </form>
                    <?php
                        // Als er een gebruiker, klant en adres is:
                    ...
                    <?php
                        // Als er een gebruiker en klant is:
                ...
                <?php
                    ...
                    echo "Oops! Iets ging mis met het uitvoeren van de statement, u word terug gestuurd.";
                    header("Refresh: 4; url=" . $_SESSION['lastpage'] . "", true, 0);
                    exit();
                }
